...
# If number of steps is not assigned use 3^rotatable torsions #
	...
	if SEARCHPARAMS.STEP == 0: SEARCHPARAMS.STEP = int(math.pow(3,FMVAR.MCNV))
###############################################################
...
# Stop once number of steps exceeded or no new conformers found	
	while CSEARCH.STEP*SEARCHPARAMS.POOL <= SEARCHPARAMS.STEP and (CSEARCH.STEP*SEARCHPARAMS.POOL-CSEARCH.LASTFOUND) <= 100:
		
# Monte Carlo Multiple Minimum Search (MCMM) 
...
# Check for acceptable VDW contacts
				NBcontacts = 1
				while NBcontacts > 0: 
